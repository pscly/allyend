...
export default function HomePage() {
  ...
  return (
    <div className="relative flex min-h-screen flex-col overflow-hidden bg-background">
      ...
      <div
        aria-hidden
        className="pointer-events-none absolute inset-0 z-0 bg-cover bg-fixed bg-center bg-no-repeat"
        style={{ backgroundImage: `url(${term.imagePath})` }}
      />
      <div
        aria-hidden
        className="pointer-events-none absolute inset-0 z-0 bg-gradient-to-b from-black/40 via-black/20 to-transparent"
      />
        ...
        <main className="relative flex-1">
          <section className="mx-auto w-full max-w-6xl px-4 py-14 md:py-20">
            <div className="grid gap-8 lg:grid-cols-[3fr_2fr] lg:items-center">
              {/* 左侧文案卡片（毛玻璃） */}
              <div className="relative rounded-3xl bg-white/10 bg-clip-padding p-6 text-white shadow-[0_12px_40px_rgba(2,6,23,0.18)] ring-1 ring-inset ring-white/15 backdrop-blur-2xl backdrop-saturate-150 dark:bg-white/10">
                ...
                <div className="mb-3 flex flex-wrap items-center gap-2">
                  <span className="bg-white/12 inline-flex items-center rounded-full px-3 py-1 text-xs text-white/90">
                    当前节气：{ctx.current.name}
                  </span>
                  <span className="bg-white/8 inline-flex items-center rounded-full px-3 py-1 text-xs text-white/85">
