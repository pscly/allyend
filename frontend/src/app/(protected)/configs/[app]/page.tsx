...
export default function ConfigDetailPage({
  ...
  const saveMutation = useMutation({
    mutationFn: async () => {
      ...
      return apiClient.put(endpoints.configs.upsert(app), {
  ...
  const statsQuery = useQuery<StatsOut>({
    ...
    queryFn: async () =>
      apiClient.get<StatsOut>(endpoints.configs.stats(app), {
        searchParams: { days: 7, granularity: "day" },
      }),
    staleTime: 10_000,
    enabled: tab === "stats",
  });
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2 text-sm text-muted-foreground">
          ...
          <span>
            更新 {detailQuery.data ? new Date(detailQuery.data.updated_at).toLocaleString() : "-"}
          </span>
        </div>
      </div>
      ...
      <div className="flex items-center gap-3">
        <button
          className={`rounded-full px-3 py-1 text-sm ${tab === "edit" ? "bg-primary text-primary-foreground" : "bg-muted"}`}
          onClick={() => setTab("edit")}
        >
          编辑配置
        </button>
        <button
          className={`rounded-full px-3 py-1 text-sm ${tab === "stats" ? "bg-primary text-primary-foreground" : "bg-muted"}`}
        ...
        <section className="space-y-4 rounded-xl border border-border/60 bg-card/50 p-4">
          ...
          <div>
            ...
            <div className="grid grid-cols-1 gap-2 md:grid-cols-2">
              ...
              {statsQuery.data?.top_ips?.length === 0 && (
