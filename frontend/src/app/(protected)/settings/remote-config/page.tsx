...
export default function RemoteConfigPage() {
  ...
  const deleteMutation = useMutation({
    mutationFn: async (app: string) =>
      apiClient.delete<{ ok: boolean }>(endpoints.configs.remove(app)),
    onSuccess: () => qc.invalidateQueries({ queryKey: ["configs"] }),
  });
  ...
  const createMutation = useMutation({
    mutationFn: async () => {
      ...
      return apiClient.put<
        AppConfigDetail,
        { description: string | null; content: Record<string, unknown> }
    ...
    <div className="space-y-6">
      ...
      <section className="rounded-xl border border-border/60 bg-card/50 p-4">
        ...
        {error ? (
          ...
          <div className="text-sm text-muted-foreground">
            在上方填写地址与 app，点击“立即获取”拉取配置。默认通过 GET 请求，无需登录；若服务端有
            CORS 限制，请改为通过后端代理。
          </div>
        )}
      </section>
      ...
      <section className="text-xs text-muted-foreground">
        ...
        <ul className="list-inside list-disc space-y-1">
          <li>
            若服务端返回加密内容，可在本页顶部的 <code>decryptConfig</code> 函数中加入解密逻辑。
          </li>
      ...
      <section className="space-y-3">
              ...
              <div className="grid gap-3">
                <div className="grid gap-2">
                  ...
                  <Input
                    id="newApp"
                    value={newApp}
                    onChange={(e) => setNewApp(e.target.value.trim())}
                    placeholder="例如：my_app_001"
                  />
                ...
                <div className="grid gap-2">
                  ...
                  <textarea
                    id="newJson"
                    className="min-h-[160px] w-full rounded-md border border-border bg-background p-2 font-mono text-sm"
                    value={newJson}
        ...
        <div className="flex flex-col gap-3 rounded-xl border border-border/60 bg-card/50 p-3">
          <div className="flex items-center gap-2">
            ...
            >
              {onlyEnabled ? <Eye className="mr-2 h-4 w-4" /> : <EyeOff className="mr-2 h-4 w-4" />}{" "}
              {onlyEnabled ? "仅启用" : "全部"}
            </Button>
            <Button
              variant="ghost"
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="text-left text-muted-foreground">
                  ...
                  <th className="whitespace-nowrap p-2">读取次数</th>
                  <th className="whitespace-nowrap p-2">更新时间</th>
                  <th className="w-[220px] p-2">操作</th>
                </tr>
              </thead>
              <tbody>
                {listQuery.isLoading ? (
                      ...
                      <td className="p-2">
                        <div className="flex items-center gap-2">
                          <Button
                            ...
                            size="sm"
                            onClick={() =>
                              togglePinnedMutation.mutate({ app: item.app, pinned: !item.pinned })
                            }
